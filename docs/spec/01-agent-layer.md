...
- **Why retrying cannot pay twice, and it is not because the implementation is careful.** An authorization carries a nonce and the token refuses a nonce it has already seen. Resending the identical signed bytes is therefore at most once by the primitive: if the first attempt actually landed, the second is refused by the contract rather than by the implementation. **Signing again is the thing that would double spend**, because a fresh signature carries a fresh nonce and is a second authorization, and that remains forbidden.
