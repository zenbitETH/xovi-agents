...
- **The free daily allowance is configured at zero on the deployment**, so a registered wallet's first read of the day settles like every other and the payment is what a visitor sees. It is a set value rather than an absence: unset, the code serves twenty free reads a day. A reader can falsify this by reading twice with a registered wallet and finding the first one free.
