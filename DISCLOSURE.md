...
- **A dashboard over one wallet's own record**: the run drawn as objects rather than sentences, the settlements it made with their explorer links, the confirmation this repository carries with the recovery a stranger can run beside it, the confirmed proposals that wallet submitted, and whether a name resolves to it. Every number on it comes from a served field or a merged document, and the sections that would need a number nobody serves carry a sentence instead. The board marks each cell that wallet's own agent has read with how far the run got, read from separate sources rather than derived from one another, and a proposal Zenbit's public list does not carry is drawn as **not seen** rather than as refused, because that list serves confirmed clips only and returns the fifty most recent. A clip this deployment has anchored is drawn as confirmed as well as attested, since the attestation is of a person's own signed decision and that record outlives the fifty. A reader can falsify this by proposing a clip and finding its cell drawn as confirmed before a person has decided.
...
- **Proof-of-human caps**, so a per-wallet limit is not defeated by generating wallets. **A free read is proven locally from the wallet's own signature and asks the facilitator nothing.** The signer of the payment authorization is recovered against the token's own EIP-712 domain and compared with the address it names, which is the same proof verification gave and costs nothing, so a registered person whose wallet holds no USDC still receives the reads their allowance grants; the authorization's nonce is spent and held for thirty days, because a free read settles nothing on chain and the same signed header would otherwise be presentable again. A reader can falsify this by making a free read from an unfunded enrolled wallet and by presenting the same header twice.
...
The rest are not features that are missing but things that have not happened. Each is two present tense statements, something this repository already does and something it does not, so the day one stops being true it is an absence that shows rather than a promise that expires. They were a tab on the page, drawn at a reader who had not yet seen the working surface they are about; the page states what it does and this document states what it does not.

- **A mainnet.** Every payment here settles on Base Sepolia. Nothing here writes to a mainnet; one read is on one.
- **The money.** Receipts land in a ledger. No rule routes any of it onward.
- **A look somebody paid for.** A human confirms or rejects every proposal and signs the decision. No institution has paid for one.
- **A query from anybody else.** The anchor joins the confirmation. No key but Zenbit's has queried it.
- **A second producer.** One colony produces every record. No second producer exists.
