...
- **The page reaches three third party origins, and which of them depends on what a person is doing.** At rest it loads one: the newest recording on offer, embedded from the host that sets no cookie, chosen over the ordinary one, which sets three on a plain fetch. It is a recording rather than the live channel, whose embed draws a dead player whenever the museum is not broadcasting, and it carries no parameter, so nothing plays until a person asks it to. On the board it also loads `i.ytimg.com`, for each on offer cell's recording thumbnail. On the World ID step it reaches World's own origins, `developer.world.org` and, under a staging configuration, World's simulator. Nothing else on the page reaches an origin Zenbit does not serve, and a reader can falsify each of the three by opening the page at rest, on the board and at that step and reading the requests the browser makes.
