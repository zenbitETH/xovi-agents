...
| A person's own surface | built: a home with the newest recording on offer and five cards saying what this is; one enrolment step, which asks whether a person stands behind the wallet and may be declined and returned to from the account; a board of what is on offer per day and per species; the run drawn one card at a time; the record as four objects, one tab each; and the account's receipts, proposals and names. Every number on it comes from a served field or a merged document |
| A cell says what it is, and where the agent has been | built: a cell on offer carries the recording's public thumbnail and length, the span its windows cover and what a read costs, read from that cell's own payment challenge, and never how many windows it holds. A wallet's own runs are recorded, and a cell it has read carries how far that run got as four events from three sources, the runs table, the run's own outcome and, for the last two, Zenbit's public list and the anchor store, none of them derived from the one before it. An anchored clip reads as confirmed as well as attested, because an attestation is of a person's own signed decision, and the public list is the source for a clip confirmed and not yet anchored. A proposal the public list does not carry is drawn as not seen and never as refused, because that list serves confirmed clips only and the fifty most recent. A payer sees its own marks and no others, and a board asked without a payer carries none |
