...
| Leg | State |
|---|---|
| Agent identity over ENS | running: `xovi.eth` is registered on Ethereum Sepolia and its `x402:windows` record resolves to the windows endpoint, read 2026-09-12 through the function the agent calls, with no injected lookup. The four refusal branches are proven against an injected resolver and the protocol guard is still owed against the real record |
